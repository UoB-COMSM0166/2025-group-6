...
export default class ObstacleHandler {
    ...
    /**
     * Generate a random time interval for next obstacle spawn (between 3s - 7s)
     * @returns {*}
     */
    getRandomSpawnTime() {
    ...
    /**
     * Update obstacles, spawn new ones at random times, and remove expired ones
     * @param gameActive
     */
    updateObstacles(gameActive) {
        if (gameActive && millis() > this.nextSpawnTime
            && (this.game.player1.score >= 3 || this.game.player2.score >= 3) && !this.hasSpawn) {
